...
if [[ "$all_present" == true && "$force" != true ]]; then
  ui_ok "OpenCode integration already installed at $target_root"
  ui_info "Nothing to do."
  exit 0
fi

if [[ "$force" == true ]]; then
  ui_info "Force reinstall enabled for OpenCode integration."
fi
...
ui_ok "Installed OpenCode commands and skill to $target_root"
...
ui_info "Restart OpenCode to load new command and skill definitions."
